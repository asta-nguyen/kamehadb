...
export function DataTable<T>({
  ...
  return (
    <div className="w-full overflow-x-auto">
      <Table className={cn('w-max min-w-full text-xs', className)}>
        {rows.length > 0 ? (
          <>
            <TableHeader className={cn(stickyHeader && 'sticky top-0 z-10 bg-muted/50 backdrop-blur')}>
              <TableRow style={{ gridTemplateColumns }} className={cn(!stickyHeader && 'bg-muted')}>
                {prefix && (
                  <TableHead
                    className={cn(defaultHeaderClass, prefixHeaderClassName, 'sticky left-0 z-30 min-w-14 bg-muted')}
                  >
                    {prefixHeader ?? ''}
                  </TableHead>
                )}
                    ...
                    >
                      ...
                      {useResize && (
                        <div
                          ...
                          className="absolute bottom-0 right-0 top-0 z-10 flex items-center justify-center w-2.5 group cursor-col-resize active:bg-primary/[0.08]"
                        >
                          <div className="w-px h-full bg-border/60 rounded-full transition-colors group-hover:bg-primary/50 group-active:bg-primary" />
                        </div>
                      )}
                    </TableHead>
                  );
                ...
                {suffix && (
                  <TableHead className={cn(defaultHeaderClass, suffixHeaderClassName, 'min-w-20')}>
                    {suffixHeader ?? ''}
                  </TableHead>
                )}
              </TableRow>
              ...
              {rows.length === 0 ? (
                ...
                rows.map((row, rowIndex) => {
                  ...
                  return (
                    ...
                    >
                      {prefix && (
                        <TableCell className={cn('sticky left-0 z-20 px-2 py-1 bg-background', prefixCellClassName)}>
                          {prefix(row, rowIndex)}
                        </TableCell>
                      )}
                      {showIndex && (
                        <TableCell className="px-3 py-0.5 text-muted-foreground">
                      ...
                      {columns.map((col) => {
                        ...
                        return (
                          <TableCell
                            key={col.id}
                            className={cn('px-2 py-1 overflow-hidden truncate', col.cellClassName)}
                            title={title}
                          >
                      ...
                      {suffix && (
                        <TableCell className={cn('px-2 py-1', suffixCellClassName)}>{suffix(row, rowIndex)}</TableCell>
                      )}
                    </TableRow>
                  );
                })
