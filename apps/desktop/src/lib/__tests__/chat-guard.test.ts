...
describe('chat send guard', () => {
  ...
  it('does not send during IME composition (isComposing)', () => {
    expect(shouldSendMessage(makeEvent({ nativeEvent: { isComposing: true } }))).toBe(false);
  });
  ...
  it('does not send on Shift+Enter even during IME', () => {
    expect(shouldSendMessage(makeEvent({ shiftKey: true, nativeEvent: { isComposing: true } }))).toBe(false);
  });

  it('does not send on held Enter even without IME', () => {
    expect(shouldSendMessage(makeEvent({ repeat: true, nativeEvent: { isComposing: false } }))).toBe(false);
  });
});
