...
export const USERNAME_REQUIRED_KINDS: readonly DbKind[] = [KIND.POSTGRES, KIND.MYSQL, KIND.MARIADB];
...
export const CreateConnectionProfileSchema = BaseCreateSchema.refine(
  (data) => {
    ...
    if (isUsernameRequired(data.kind) && (!data.username || !data.username.trim())) {
      return false;
    }
  ...
  (data) => {
    ...
    if (isUsernameRequired(data.kind) && (!data.username || !data.username.trim())) {
      return {
        message: `Username is required for ${data.kind} connections`,
        path: ['username'],
      };
    }
    return {
